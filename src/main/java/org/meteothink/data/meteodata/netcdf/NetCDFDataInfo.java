...
import org.meteothink.ndarray.ArrayInt;
 ...
 * @author yaqiang
 */
public class NetCDFDataInfo extends DataInfo {
    ...
    private List<ucar.nc2.Attribute> ncAtributes = new ArrayList<>();
     ...
     *
     * @return Netcdf global attributes
     */
    public List<ucar.nc2.Attribute> getNCGlobalAttributes() {
        return this.ncAtributes;
    }
...
//                        dim.setShortName(newName);
            ...
            ncAtributes = ncfile.getGlobalAttributes();
            attributes = new ArrayList<>();
            for (ucar.nc2.Attribute attr : this.ncAtributes) {
                attributes.add(NCUtil.convertAttribute(attr));
            }
            ...
            for (ucar.nc2.Variable var : ncVariables) {
                ...
                for (ucar.nc2.Dimension dim : var.getDimensions()) {
                    ...
                    if (idx >= 0) {
                        Dimension ndim = dimensions.get(idx);
    ...
    public ucar.nc2.Attribute findGlobalAttribute(String attName) {
        for (ucar.nc2.Attribute att : this.ncAtributes) {
    ...
    private Conventions getConvention() {
        ...
        for (ucar.nc2.Attribute aAtts : ncAtributes) {
    ...
    private void getProjection() {
        CoordinateReferenceSystem projInfo = this.getCRS();
        switch (_convention) {
            case CF:
        ...
        this.setCRS(projInfo);
    }

    private CoordinateReferenceSystem getProjection_CF() {
                    ...
                    switch (attStr) {
                        ...
                        case "polar_stereographic":
                            ...
                            if (pVar.findAttribute("scaling_factor") != null) {
                                ...
                                //projStr += "+lat_ts=" + stPs;
                                double k0 = ProjUtil.calScaleFactorFromStandardParallel(Double.parseDouble(stPs));
        ...
        try {
            CoordinateReferenceSystem projInfo = ProjUtil.factory(projStr);
            return projInfo;
        } catch (Exception e) {
    ...
    private CoordinateReferenceSystem getProjection_IOAPI() {
        String projStr = this.getCRS().getParameterString();
        ...
        return ProjUtil.factory(projStr);
    }

    private CoordinateReferenceSystem getProjection_WRFOUT() {
        String projStr = this.getCRS().getParameterString();
    ...
    private String getGlobalAttStr(String attName) {
        String attStr = "";
        for (ucar.nc2.Attribute aAttS : this.ncAtributes) {
            ...
            if (var.getRank() == 1) {
                ...
                switch (dimType) {
                    case X:
                        ...
                        if (ProjUtil.isLonLat(this.getCRS())) {
...
//        }
    ...
    private void getDimValues_WRF(NetcdfFile ncfile) throws ParseException, IOException {
        ...
        if (_yVar != null && _xVar != null) {
            ...
            ProjUtil.reprojectPoints(points, fromProj, this.getCRS(), 0, 1);
            orgX = points[0][0];
            orgY = points[0][1];
        } else {
    ...
    public String generateInfoText() {
        ...
        for (i = 0; i < ncAtributes.size(); i++) {
            aAttS = ncAtributes.get(i);
        ...
        for (i = 0; i < ncVariables.size(); i++) {
            dataInfo += System.getProperty("line.separator") + "\t" + ncVariables.get(i).getDataType().toString()
                    + " " + ncVariables.get(i).getShortName() + "(";
...
//        }
     ...
     * @return Array data
     */
    public Array read(String varName, int[] origin, int[] size) throws ucar.ma2.InvalidRangeException {
        try {
            if (ncfile == null) {
                ncfile = NetcdfFile.open(this.getFileName());
            }
     ...
     * @return Array data
     */
    public Array read_pack(String varName, int[] origin, int[] size) throws ucar.ma2.InvalidRangeException {
        try {
            if (ncfile == null) {
                ncfile = NetcdfFile.open(this.getFileName());
            }
    ...
    public Array read(String varName, String key) throws ucar.ma2.InvalidRangeException {
        try {
            if (ncfile == null) {
                ncfile = NetcdfFile.open(this.getFileName());
            }
     ...
     */
    public static void joinDataFiles(List<String> inFiles, String outFile, String tDimName) throws ucar.ma2.InvalidRangeException {
     ...
     */
    public static void joinDataFiles_Time(List<String> inFiles, String outFile, String timeDimStr) throws IOException, InvalidRangeException, ParseException, ucar.ma2.InvalidRangeException {
        //Check number of selected files
        int fNum = inFiles.size();
        if (fNum < 2) {
            JOptionPane.showMessageDialog(null, "There should be at least two files!", "Error", JOptionPane.ERROR_MESSAGE);
        ...
        //Define global attributes
        for (ucar.nc2.Attribute attr : aDataInfo.ncAtributes) {
            ncfilew.addGroupAttribute(null, attr);
        }
     ...
     * @throws java.text.ParseException
     */
    public static void joinDataFiles_Time_pack(List<String> inFiles, String outFile, String timeDimStr) throws IOException, InvalidRangeException, ParseException, ucar.ma2.InvalidRangeException {
        //Check number of selected files
        int fNum = inFiles.size();
        if (fNum < 2) {
            JOptionPane.showMessageDialog(null, "There should be at least two files!", "Error", JOptionPane.ERROR_MESSAGE);
     ...
     * @throws org.meteothink.ndarray.InvalidRangeException
     */
    public static void joinDataFiles_Variable(List<String> inFiles, String outFile) throws IOException, InvalidRangeException, ucar.ma2.InvalidRangeException {
        //Check number of selected files
        int fNum = inFiles.size();
        if (fNum < 2) {
            JOptionPane.showMessageDialog(null, "There should be at least two files!", "Error", JOptionPane.ERROR_MESSAGE);
     ...
     */
    public static void addTimeDimension(String inFile, String outFile, Date aTime) throws ucar.ma2.InvalidRangeException {
        try {
     ...
     */
    public static void addTimeDimension(String inFile, String outFile, Date aTime, String timeUnit) throws ParseException, IOException, InvalidRangeException, ucar.ma2.InvalidRangeException, ucar.ma2.InvalidRangeException, ucar.ma2.InvalidRangeException {
        //Set data info
        NetCDFDataInfo aDataInfo = new NetCDFDataInfo();
        ...
        ncfilew.write(tvar, NCUtil.convertArray(timeValue));
