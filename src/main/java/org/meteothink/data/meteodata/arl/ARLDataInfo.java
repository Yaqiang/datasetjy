 ...
 */
package org.meteothink.data.meteodata.arl;
 ...
 * @author Yaqiang Wang
 */
public class ARLDataInfo extends DataInfo {
...
//                            + "+lon_0=" + String.valueOf(aDH.REF_LON + aDH.ORIENT);
                ...
                CoordinateReferenceSystem crs = ProjUtil.factory(ProjStr);
                this.setCRS(crs);

                //Set X Y
                X = new double[aDH.NX];
                Y = new double[aDH.NY];
                getProjectedXY(crs, aDH.SIZE * 1000, aDH.SYNC_XP, aDH.SYNC_YP, aDH.SYNC_LON,
                        aDH.SYNC_LAT, X, Y);
            }
            ...
            for (Variable var : variables) {
                ...
                if (len > 1){
                    ...
                    if (zDim == null){
                        ...
                        zDim.setShortName("Z_" + String.valueOf(len));
    ...
    private void getProjectedXY(CoordinateReferenceSystem crs, float size,
            float sync_XP, float sync_YP, float sync_Lon, float sync_Lat,
            double[] X, double[] Y) {
        //Get sync X/Y
        CoordinateReferenceSystem fromCRS = KnownCoordinateSystems.geographic.world.WGS1984;
        ...
        ProjUtil.reprojectPoints(points, fromCRS, crs, 0, 1);
        sync_X = points[0][0];
        sync_Y = points[0][1];
     ...
     */
    public DataHead getDataHead(CoordinateReferenceSystem crs, String model, int kFlag, int icx, short mn) {
        this.setIndexRecPos();
        ...
        if (ProjectionName.getName(crs.getProjection().getName()) == ProjectionName.LongLat) {
            aDH.POLE_LAT = 90;
            aDH.POLE_LON = 0;
            ...
            if (Double.isNaN(crs.getProjection().getProjectionLongitudeDegrees())) {
                sync_x = 1;
                sync_y = 1;
                ...
                CoordinateReferenceSystem toCRS = KnownCoordinateSystems.geographic.world.WGS1984;
                ProjUtil.reprojectPoints(points, crs, toCRS, 0, 1);
                sync_lon = points[0][0];
                sync_lat = points[0][1];
            } else {
                sync_x = 0.5f * (X.length + 1);
                sync_y = 0.5f * (Y.length + 1);   
                sync_lon = crs.getProjection().getProjectionLongitudeDegrees();
                sync_lat = crs.getProjection().getProjectionLatitudeDegrees();
            }
            Projection aProj = crs.getProjection();
            ...
            switch (ProjectionName.getName(crs.getProjection().getName())) {
